...
@Component({
  selector: 'app-admin-department-manager-form',
  templateUrl: './department-manager-form.component.html',
  styleUrl: './department-manager-form.component.scss',
  standalone: true,
...
export class DepartmentManagerFormComponent {
  @Input() visible: boolean = false;
  @Input() dialogHeader: string = '';
  @Input() submitted: boolean = false;
  @Input() Specialities: specialityDTO[] = [];
  @Output() hideDialog = new EventEmitter<void>();
  @Output() saveDepartmentManager = new EventEmitter<DoctorDTO>();
  form: FormGroup;
  ...
  constructor(private fb: FormBuilder) {
    this.form = this.fb.group({
      ...
      specialityDTO: ['', Validators.required]
    });
  }
  ...
  onSpecialityChange(event: any) {
    ...
    this.form.get('specialityDTO')?.setValue(selectedSpeciality);  
  }
  ...
  handleSave() {
    if (this.form.valid) {
      this.saveDepartmentManager.emit(this.form.value);
    }
  }
