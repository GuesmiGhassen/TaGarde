...
export class HospitalDepartmentManagerGMService {
    ...
    create(item: DoctorDTO, role: string): Observable<ApiResponse<DoctorDTO>> {
        const token = this.auth.getToken(); // Replace with your actual token retrieval method
    ...
    delete(id: string): Observable<void> {
        const HospitalId = sessionStorage.getItem("HospitalId");
        console.log(id)
        return this.http.put<void>(`http://localhost:8081/api/v1/hospital-department/${HospitalId}/hospital-departments/${id}/manager/remove`, null).pipe(
            catchError(this.handleError)
        );
    }
